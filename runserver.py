...
def dbSetup():
    ...
    try:
        ...
        r.db('rcrdkeeprapp').table('record_condition').insert({'abbr':'P','condition':'Poor', 'order': 4}).run(connection)
        r.db('rcrdkeeprapp').table('record_condition').insert({'abbr':'VP','condition':'Very Poor', 'order': 5}).run(connection)
...
if __name__ == "__main__":
    ...
    if args.run_setup:
        ...
        app.run(host='10.0.0.23', port=4000, debug=True)
